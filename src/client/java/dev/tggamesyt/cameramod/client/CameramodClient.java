...
public class CameramodClient implements ClientModInitializer {
    ...
    public static void syncGameRuleBoolToServer(
            net.minecraft.world.rule.GameRule<Boolean> key, boolean value) {
        ...
        server.execute(() -> {
            net.minecraft.world.rule.GameRules rules = server.getSaveProperties().getGameRules();
            if (rules.getValue(key) != value) rules.setValue(key, value, server);
        });
    }

    /** Integer-rule counterpart of {@link #syncGameRuleBoolToServer}. */
    public static void syncGameRuleIntToServer(
            net.minecraft.world.rule.GameRule<Integer> key, int value) {
        net.minecraft.server.MinecraftServer server = MinecraftClient.getInstance().getServer();
        if (server == null || server.getCurrentPlayerCount() > 1) return;
        server.execute(() -> {
            net.minecraft.world.rule.GameRules rules = server.getSaveProperties().getGameRules();
            if (!rules.getValue(key).equals(value)) rules.setValue(key, value, server);
        });
    }
