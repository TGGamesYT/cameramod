...
import static dev.tggamesyt.cameramod.Cameramod.CAMERA_ENTITY_ENTITY_TYPE;
...
public class ServerItems {
    ...
    static RegistryKey<Item> camera_remover_key = RegistryKey.of(RegistryKeys.ITEM, Identifier.of(Cameramod.MOD_ID, "camera_remover"));
    public static final Item CAMERA_REMOVER = new CameraRemoverItem(new Item.Settings().maxCount(1).registryKey(camera_remover_key));
    ...
    public static void registerItems() {
        ...
        Registry.register(Registries.ITEM, camera_remover_key, CAMERA_REMOVER);
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            boolean seesChat = server.getSaveProperties().getGameRules().getValue(Cameramod.CAMERA_SEES_CHAT);
            boolean flipped = server.getSaveProperties().getGameRules().getValue(Cameramod.CAMERA_FLIPPED);
            boolean nameTags = server.getSaveProperties().getGameRules().getValue(Cameramod.CAMERA_NAME_TAGS);
            boolean guiMode = server.getSaveProperties().getGameRules().getValue(Cameramod.CAMERA_GUI_MODE);
            boolean showPlayerGuis = server.getSaveProperties().getGameRules().getValue(Cameramod.CAMERA_SHOW_PLAYER_GUIS);
            ...
            int streamFps  = server.getSaveProperties().getGameRules().getValue(Cameramod.CAMERA_STREAM_FPS);
            int virtualFps = server.getSaveProperties().getGameRules().getValue(Cameramod.CAMERA_VIRTUAL_FPS);
